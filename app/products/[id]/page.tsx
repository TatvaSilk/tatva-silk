...
async function getProduct(id: string): Promise<Detail | null> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL ?? ''}/api/store/products/${id}`, { cache: 'no-store' });
  ...
  return (
    <main style={{ padding: '40px 24px', maxWidth: 1080, margin: '0 auto' }}>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginTop: 8 }}>
        {/* Left: gallery */}
        <div>
          <div style={{ width:'100%', height: 420, borderRadius: 8, overflow:'hidden', border: '1px solid #eee', marginBottom: 12 }}>
          ...
          {p.images?.length > 1 && (
            <div style={{ display:'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(90px, 1fr))', gap: 8 }}>
              {p.images.slice(1).map((img, i) => (
                <div key={i} style={{ width:'100%', height: 90, overflow:'hidden', borderRadius: 6, border:'1px solid #eee' }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        <div>
          ...
          {p.description ? <div style={{ marginTop: 16, lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{p.description}</div> : null}
        </div>
      </section>
    </main>
  );
