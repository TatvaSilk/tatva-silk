...
import { useEffect, useState, Suspense } from 'react';   // ⬅️ add Suspense here
      ...
      <header style={{ background: '#131921', color: '#fff' }}>
        ...
        >
          ...
          <form onSubmit={onSearch} style={{ display: 'flex', flex: 1, maxWidth: 520 }}>
            ...
            <button
              type="submit"
              style={{ padding: '0 16px', background: '#febd69', border: 'none',
                       borderRadius: '0 6px 6px 0', fontWeight: 600, cursor: 'pointer' }}
            >
              Search
            </button>
          </form>
            ...
            {menuOpen && (
              <div
                style={{
                  ...
                  background: '#fff', color: '#111', borderRadius: 8, minWidth: 220,
                  padding: 12, zIndex: 9999, boxShadow: '0 10px 30px rgba(0,0,0,.25)',
                }}
              >
                {userEmail ? (
                  <button
        ...
        {/* ====== TWO‑LEVEL CATEGORY BAR (wrap in Suspense) ====== */}
